...
class Authy::Client
  ...
  def block!
    ...
    self.save
    Authy::OauthToken.block_client!(self.uri)
    Authy::OauthAuthorization.block_client!(self.uri)
  end
  ...
  class << self
    ...
    def sync_clients_with_scope(scope)
      Authy::Client.all.each do |client|
        scope_string = client.scope.join(Authy.settings["scope_separator"])
        client.scope_values = Authy.normalize_scope(scope_string)
        client.save
      end
    end
  end
  ...
  private
    ...
    def block_tokens!
      Authy::OauthToken.where(client_uri: uri).map(&:block!)
    end

    def block_authorizations!
      Authy::OauthAuthorization.where(client_uri: uri).map(&:block!)
    end

    def random_secret
      self.secret = ActiveSupport::SecureRandom.hex(Authy.settings["random_length"])
    end

    def clean
      Authy::OauthToken.where(client_uri: uri).destroy_all
      Authy::OauthAuthorization.where(client_uri: uri).destroy_all
    end
