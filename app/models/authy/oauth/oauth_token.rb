...
class Authy::OauthToken
  ...
  private
    ...
    def random_token
      self.token = ActiveSupport::SecureRandom.hex(Authy.settings["random_length"])
    end

    def random_refresh_token
      self.refresh_token = ActiveSupport::SecureRandom.hex(Authy.settings["random_length"])
    end

    def create_expiration
      self.expire_at = Chronic.parse("in #{Authy.settings["token_expires_in"]} seconds")
    end
