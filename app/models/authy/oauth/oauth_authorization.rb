...
class Authy::OauthAuthorization
  ...
  private
    ...
    def random_code
      self.code = ActiveSupport::SecureRandom.hex(Authy.settings["random_length"])
    end

    # expiration time
    def create_expiration
      self.expire_at = Chronic.parse("in #{Authy.settings["authorization_expires_in"]} seconds")
    end
